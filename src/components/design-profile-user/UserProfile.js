...
export default function UserProfile(props) {
  const flag = countries.find(
    (country) => country.label === props.user?.country
  );
  return (
    <Stack
      spacing={1}
      sx={{ p: 1.2, width: "100%", bgcolor: "#f6f2f6", borderRadius: "10px" }}
    >
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        >
          <Link to={props.user?.job ? "/jobs/" + props.user?.job._id : ""}>
            {props.user?.job && props.user?.job.title
              ? props.user?.job.title.toUpperCase()
              : "Job"}
          </Link>
        </Typography>
      </Box>
