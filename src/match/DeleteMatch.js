...
export default function DeleteMatch(props) {
  ...
  const deleteMatch = () => {
    setProgress(true);
    remove({ matchId: props.match._id }, { t: jwt.token }).then(
      (data) => {
        if (data.error) {
          setIsError({
            ...isError,
  ...
  return (
    <span>
      ...
      <Dialog open={open} onClose={handleRequestClose}>
        ...
        <DialogContent>
          <DialogContentText>Confirm to delete this match.</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleRequestClose} color="primary">
            Cancel
          </Button>
          <Button onClick={deleteMatch} color="secondary" autoFocus="autoFocus">
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
